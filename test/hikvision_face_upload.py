...
def diagnose_permission(client: HikvisionClient, employee_no: str, door_no: int) -> int:
    issues = []
    try:
        user = client.get_user_by_employee_no_full(employee_no)
    except Exception as exc:
        print(json.dumps({"ok": False, "error": f"Failed to read user from camera: {exc}"}, ensure_ascii=False))
        return 2

    if user is None:
        print(json.dumps({"ok": False, "error": "User not found on camera"}, ensure_ascii=False))
        return 2

    num_of_face = user.get("numOfFace")
    if isinstance(num_of_face, int) and num_of_face <= 0:
        issues.append("User has zero enrolled faces (numOfFace <= 0)")

    door_right_raw = str(user.get("doorRight", "")).strip()
    allowed_doors = {x.strip() for x in door_right_raw.split(",") if x.strip()}
    if str(door_no) not in allowed_doors:
        issues.append(f"doorRight does not include door {door_no} (doorRight={door_right_raw!r})")

    right_plan = user.get("RightPlan") or []
    has_door_plan = any(str(item.get("doorNo")) == str(door_no) for item in right_plan if isinstance(item, dict))
    if not has_door_plan:
        issues.append(f"RightPlan does not include doorNo={door_no}")

    valid = user.get("Valid") or {}
    if valid.get("enable") is not True:
        issues.append("Valid.enable is not true")

    begin_s = valid.get("beginTime")
    end_s = valid.get("endTime")
    begin_dt = _parse_camera_time(begin_s)
    end_dt = _parse_camera_time(end_s)

    camera_now = None
    camera_time_payload = None
    try:
        camera_time_payload = client.get_isapi_json("ISAPI/System/time?format=json")
        camera_now_str = (
            camera_time_payload.get("Time", {}).get("localTime")
            or camera_time_payload.get("Time", {}).get("time")
        )
        camera_now = _parse_camera_time(camera_now_str)
    except Exception:
        camera_now = None

    if camera_now is not None and begin_dt is not None and camera_now < begin_dt:
        issues.append(f"Current camera time is before beginTime ({camera_now} < {begin_dt})")
    if camera_now is not None and end_dt is not None and camera_now > end_dt:
        issues.append(f"Current camera time is after endTime ({camera_now} > {end_dt})")

    verify_mode = user.get("userVerifyMode")
    summary = {
        "ok": len(issues) == 0,
        "employeeNo": employee_no,
        ...
        "userInfo": {
            ...
            "name": user.get("name"),
            "numOfFace": user.get("numOfFace"),
            "doorRight": user.get("doorRight"),
            "RightPlan": user.get("RightPlan"),
            "Valid": user.get("Valid"),
            "userVerifyMode": verify_mode,
        },
        "cameraTime": camera_time_payload,
        "hint": "If no issues are listed but camera still denies access, check device auth mode (e.g. face+card) and template/door mapping in Hikvision UI.",
    }
...
def main() -> None:
    ...
    if args.cmd == "diagnose":
        raise SystemExit(diagnose_permission(client, args.employee_no, args.door_no))
